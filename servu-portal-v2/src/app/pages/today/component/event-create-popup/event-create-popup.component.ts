...
export class EventCreatePopupComponent implements OnInit {
    ...
    constructor(public cs: CustomerService,
        ...
        cs.getList().subscribe(l => {
            this.customers = Object.assign([], l);
        });

    }

    onCustomerCreate(l) {
        this.customerModal = l.name;
    }
    ...
    onProductSelect(p) {
        ...
        if (p) {
            ...
            this.ps.getProductStaff(p.map(r => {
                ...
                this.providers = l;
                this.dateSelect(this.selectedDate.clone());
            });
    ...
    selectProvider(providerID) {
        if (providerID) {
            this.loadSlots({date: this.selectedDate.clone()}, providerID);
        }
    }
