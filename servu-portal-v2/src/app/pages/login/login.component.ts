...
export class Login {
    ...
    constructor(fb: FormBuilder, public auth: AuthRest, public router: Router, public bs: BusinessRest) {
        this.form = fb.group({
    ...
    public onSubmit(values: any): void {
        ...
        if (this.form.valid) {
            this.auth.login(values).subscribe((resutl) => {
                this.bs.getBranchesList().subscribe((l) => {
                    var branch = _.find(l.data, (b: any) => {
                        return b.business_user[0].is_default
                    });
                    this.auth.branchDetails(branch);
