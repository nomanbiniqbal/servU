...
export class ProductService {
    ...
    addProductStaff(val: any) {
        return this._pr.postProductStaff(val).do(r => {
            var p = r.data;
            var find: any = _.find(this._productStaffData, {id: p.business_user_id});
            if (find) {
                find.business_user_product.push(p)
            }
            this._pStaffSubject.next(this._productStaffData);
        });
    }
    ...
    deleteProductStaff(staff: any) {
        return this._pr.deleteProductStaff(staff.id).do(r => {
            let st: any = _.find(this._productStaffData, {id: staff.business_user_id});
            var p = _.find(st.business_user_product, {business_product_id: staff.id});
            let index = _.indexOf(staff.business_user_product, p);
            st.business_user_product.splice(index, 1);
            this._pStaffSubject.next(this._productStaffData);
        });
    }
